...
if(os.path.exists('d3d9.dll')):
    print(colorama.Back.MAGENTA+'Opening d3d9.dll for analysis'+colorama.Style.RESET_ALL)
    stream = os.popen('certutil -hashfile d3d9.dll MD5')
    output = stream.read()
    my_arc = output.split('\n')[1]
    ...
    print(colorama.Back.RED+"ERR"+colorama.Style.RESET_ALL+" We can't find a current d3d9.dll in this directory.\nWould you like to download one instead?\n(y/n)")
    ans = input()
    if(str(ans).lower() != 'y'):
